...
void WindowManager::openWidget(const QString& id)
{
	if (id == WidgetID::SCREENSHOT) {
		//QEventLoop loop;
		//loop.processEvents();
		ScreenshotWidget* prevScreenshot = screenshot_.release();
		if (prevScreenshot) { prevScreenshot->hide(); }
		defer(if (prevScreenshot) prevScreenshot->deleteLater(););

		screenshot_ = std::make_unique<ScreenshotWidget>();
		...
		if (widgets_.contains(id)) {
			...
			title->setTitle(QStringLiteral("设置"));
