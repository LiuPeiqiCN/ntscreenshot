...
QPoint SelectedScreenWidget::adjustPos(QPoint p)
{
	...
	else if (p.x() + this->width() > QApplication::primaryScreen()->geometry().width()) {
		p.setX(QApplication::primaryScreen()->geometry().width() - this->width());
	}
	if (p.y() < 0) {
		p.setY(0);
	}
	else if (p.y() + this->height() > QApplication::primaryScreen()->geometry().height()) {
		p.setY(QApplication::primaryScreen()->geometry().height() - this->height());
	}
	return p;
}
...
QRect SelectedScreenWidget::adjustRect(QRect r)
{
	...
	if (r.right() > QApplication::primaryScreen()->geometry().width()) {
		r.setRight(QApplication::primaryScreen()->geometry().width());
	}
	if (r.top() < 0) {
		r.setTop(0);
	}
	if (r.bottom() > QApplication::primaryScreen()->geometry().height()) {
		r.setBottom(QApplication::primaryScreen()->geometry().height());
	}
	return r;
}
