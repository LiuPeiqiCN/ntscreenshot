...
void DrawPanel::adjustPos()
{
	...
	if (x + this->width() > QApplication::primaryScreen()->geometry().width()) {
		x = QApplication::primaryScreen()->geometry().width() - this->width();
	}
	if (y + this->height() > QApplication::primaryScreen()->geometry().height()) {
